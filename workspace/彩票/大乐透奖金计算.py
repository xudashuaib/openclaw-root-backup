...
规则（2026年新规则，固定奖）:
  九等奖: 3前区+0后区 或 2前区+1后区 或 1前区+2后区 或 0前区+2后区 → 5元
  八等奖: 1前区+2后区 或 0前区+2后区 → 15元
  七等奖: 3前区+1后区 或 2前区+1后区 → 100元
  六等奖: 3前区+2后区 或 2前区+2后区 → 200元
  五等奖: 4前区+0后区 或 3前区+2后区 → 300元
  四等奖: 4前区+1后区 → 3000元
  三等奖: 5前区+0后区 或 4前区+2后区 → 浮动
  二等奖: 5前区+1后区 → 浮动
  一等奖: 5前区+2后区 → 浮动

浮动奖（一二三等奖）需要当期销售额和奖池数据，本脚本返回"浮动"。

追加投注：每注多加1元，中得一/二/三等奖时额外获得基本奖金×80%。

用法:
  python3 大乐透奖金计算.py "01 02 03 04 05 + 06 07" "24 25 27 29 34 + 02 06"
  python3 大乐透奖金计算.py 01 02 03 04 05 06 07  # 空格分隔，不带+
...
def calc(bet_str, draw_str, 追加=False):
    """计算中奖等级和奖金
    返回 (奖等名称, 基本奖金, 追加奖金/0)
    """
    ...
    if front_hit == 5 and back_hit == 2:
        ...
        base = 200
        extra = int(base * add_ratio) if add_ratio else 0
    elif front_hit == 3 or (front_hit == 2 and back_hit == 1):
        label = "七等奖"
        base = 100
        extra = int(base * add_ratio) if add_ratio else 0
...
def main():
    if len(sys.argv) < 3:
        print("用法: python3 大乐透奖金计算.py <投注号码> <开奖号码> [追加]")
    ...
    bet_str = sys.argv[1]
    draw_str = sys.argv[2]
    追加 = len(sys.argv) > 3 and sys.argv[3] == "追加"

    label, base, extra = calc(bet_str, draw_str, 追加)
